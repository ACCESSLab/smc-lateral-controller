...
Controller::Controller(const rclcpp::NodeOptions & node_options) : Node("controller", node_options)
{
  ...
  switch (lateral_controller_mode) {
    ...
    case LateralControllerMode::SMC: {
      lateral_controller_ = std::make_shared<smc_lateral_controller::SmcLateralController>(*this);
      break;
    }
    default:
  ...
  const std::string & controller_mode) const
{
  ...
  if (controller_mode == "smc") return LateralControllerMode::SMC;
